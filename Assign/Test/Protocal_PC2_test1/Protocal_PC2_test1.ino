...
void loop() {
  if (state == 0) { // wait control 3 pic
    ...
    if (Serial.available() > 0) {
      order = Serial.read();
      if (order == 'g') {
        Serial.print("end state 2 from Arduino");
        state++;
      }
      else {
        pos[picCount] = Serial.read();
        pos[picCount] = pos[picCount] & B1111;
        picCount++;
      }
    }
  }
  else if (state == 3) { // send data
    if (sendPic < 3) {
      if (getAck == 0) {
  ...
  else if (state == 4) { // wait control (choose)
  ...
  else if (state == 5) { // take pic (choose)
    if (Serial.available() > 0) {
      order = Serial.read();
    }
  ...
  else if (state == 6) { // wait color from pc2
    if (Serial.available() > 0) {
      if (Serial.read() == 'g') {
        state++;
      }
        ...
        color[colorCount] = color[colorCount] & B11111111;
        colorCount++;
      }
    }
  }
...
void setFrame(int format) {
  ...
  else if (format == 17) { // color header
    dataSend |= B0100000;
    parity += 1;
  }
}
