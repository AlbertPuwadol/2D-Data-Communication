...
void loop() {
  if (state == 0) { // wait control 3 pic
    ...
    if (millis() - trashTime > 2000) { // Clear Noise
      //Serial.println("clear");
      clearData();
    }
    if (millis() - checkTime > 8000 && ACKsend == true) {
      state++;
      Serial.print(command);
    }
  }
  else if (state == 1) { // take 3 pic
  ...
  else if (state == 3) { // wait control (choose)
    ...
    if (millis() - checkTime > 13000 && ACKsend == true) {
      state++;
//    Serial.print(command);
    }
  }
  else if (state == 4) { // take pic (choose)
    ...
    if (order == 'o') {
      state++;
      Serial.println("Welcome to State 5");
    }
  ...
  else if (state == 5) { // wait color from pc2
    if (colorCount < 48) {
//      Serial.println("if");
      if (Serial.available() > 0) {
//        Serial.println("available");
        color[colorCount] = Serial.read();
        colorCount++;
//        Serial.println(colorCount);
//        Serial.println(color[colorCount]);
      }
    }
    else {
      state++;
...
/* ===================== 5_Send_Color ========================== */
void sending_Color(int head, int seqMode, int dataMode) {
  sendData(flag);
  delay(5);
  markData(head, seqMode, dataMode);
  sendData_Color(dataSend);
  delay(5);
  sendData(flag);
  delay(5);
}
...
void translateBit() {
  if (header == 0) {
    ...
    else if (dataReciver == 15) { // pos
      curSeq_Sen++;
      delay(2);
      sendPic++;
      getAck = 0;
      Serial.println("Receiver ack");
    }
    else if (dataReciver == 5) { // color 
      curSeq_Sen++;
      delay(2);
      sendColor++;
      getAck = 0;
      Serial.println(sendColor);
      Serial.println("Receiver Color ack");
    }
  }
  else if (header == 3) {
    // data from PC2
